...
基础信息:ssh分为私钥和公钥,使用的时候将公钥上传到需要登录的服务器,github账号中;

##### 工作流程
1. 本机向目标服务器发起一个请求
2. 目标服务器/github账号会用公钥加密一个随机字符串,发回客户端
3. 本机收到加密后的字符串,就会用本地的私钥进行解密
4. 本机将解密后的字符串发回目标服务器(通常也会附带上当前对话ID的哈希)
5. 目标服务器对比之后,如果匹配就会允许登录

##### 生成与配置

生成密钥:`ssh-keygen -t ed25519 -C "邮箱或备注"`

生成过程中会产生三个问题:  
1. `Enter file in which to save the key`询问存放位置
2. `Enter passphrase`询问是否给密钥设置密码
3. 重复密码  
生成成功会提示密钥存放的地址,其中带有pub后缀的是公钥

配置到github  
github的设置中有专门的ssh选项,可以自由配置多个公钥,也可以随便移除

配置到linux服务器中  
1. 全自动
   使用`ssh-copy-id [用户名]@服务器IP`可以自动化配置ssh公钥
2. 手动复制
   复制本地的公钥,登录到服务器上  
   执行以下命令
  ```
  ...
  chmod 700 ~/.ssh  #权限配置不要随便配置,可能会出现问题
  chmod 600 ~/.ssh/authorized_keys
  ```
3. 如果想要移除就删除对应文件中对应的那一行公钥就行

##### 好处
1. ssh密钥够长,一般无法暴力破解
2. 私钥不会出现在网络传输之中,被抓包也安全
